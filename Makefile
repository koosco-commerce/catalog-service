...
k8s-status: ## Show Kubernetes resources status
	...
	@kubectl get pods -n commerce -l app=catalog-service-mariadb
	@echo ""
	@echo "$(GREEN)Services:$(NC)"
	@kubectl get svc -n commerce -l service=catalog-service
	@echo ""
	@echo "$(GREEN)HPA:$(NC)"
	@kubectl get hpa -n commerce -l app=catalog-service
	@echo ""
	...
	@kubectl get deployments -n commerce -l app=catalog-service-mariadb
	@echo ""
	@echo "$(GREEN)PVC (Persistent Volume Claims):$(NC)"
	@kubectl get pvc -n commerce -l app=catalog-service-mariadb
...
k8s-scale: ## Scale application (usage: make k8s-scale REPLICAS=3)
	...
	@kubectl scale deployment/catalog-service -n commerce --replicas=$(REPLICAS)
