# Kubernetes Deployment Guide for Catalog Service

## Overview
Complete Kubernetes configuration for the catalog-service microservice with MariaDB database.
...
kubectl exec deployment/mariadb -n commerce -- \
  mysqldump -uroot -proot commerce-catalog > backup.sql
```

### Restore Database
```bash
kubectl exec -i deployment/mariadb -n commerce -- \
  mysql -uroot -proot commerce-catalog < backup.sql
```

### Access Database Shell
```bash
kubectl exec -it deployment/mariadb -n commerce -- \
  mysql -uadmin -padmin1234 commerce-catalog
```
...
kubectl set image deployment/catalog-service \
  catalog-service=catalog-service:v2.0.0 -n commerce

# Watch rollout status:
kubectl rollout status deployment/catalog-service -n commerce
```

### Rollback
```bash
kubectl rollout undo deployment/catalog-service -n commerce

# Rollback to specific revision:
kubectl rollout undo deployment/catalog-service -n commerce --to-revision=2
```
...
# Test connection from catalog-service pod
kubectl exec -it deployment/catalog-service -n commerce -- \
...
```bash
# Recent logs
kubectl logs deployment/catalog-service -n commerce --tail=100

# Follow logs
kubectl logs -f deployment/catalog-service -n commerce

# Logs from all replicas
kubectl logs -l app=catalog-service -n commerce
```
