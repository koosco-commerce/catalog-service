...
@Configuration
class KafkaProducerConfig(@Value("\${spring.kafka.bootstrap-servers}") private val bootstrapServers: String) {

    private val logger = LoggerFactory.getLogger(KafkaProducerConfig::class.java)

    @PostConstruct
    fun init() = logger.info("Kafka Bootstrap Servers: $bootstrapServers")
